            ...
            <div class="form-step" id="step3" style="display: none;">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="specialRequests" rows="3"
                        placeholder="Contoh: Meja dekat jendela, perayaan ulang tahun, dll."></textarea>
                </div>

                <div class="payment-section">
                    <h3>Pembayaran DP, minimal Rp 50.000</h3>

                    <div class="qris-box">
                        <img src="assets/images/qris.png" alt="QRIS Coffee Shop" class="qris-image">
                    </div>

                    <p class="dp-info">
                        Silakan transfer DP dan upload bukti pembayaran di bawah ini.
                    </p>

                    <div class="form-group">
                        <label>Upload Bukti Transfer</label>
                        <input type="file" id="dpProof" accept="image/*" required>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadAvailableTables() {
            ...
            try {
                const response = await fetch(
                    `api/reservations.php?available_tables=1&date=${selectedDate}&time=${selectedTime}&party_size=${selectedPartySize}`
                );
        ...
        function displayAvailableTables(availableTables) {
            ...
            container.innerHTML = availableTables.map(table => {
                ...
                const photoUrl = table.photo && table.photo !== '' ?
                    `uploads/tables/${table.photo}` :
                    'assets/img/no-image.png';

                return `
            <div class="table-option available" data-table="${table.id}">
                
                <img src="${photoUrl}" class="table-photo">

                <h4>Meja ${table.table_number}</h4>
                <p>${table.capacity} orang</p>
                <small>${locationIcon} ${locationText}</small>
            </div>
        `;
            }).join('');
            ...
            document.querySelectorAll('.table-option').forEach(option => {
                option.addEventListener('click', function() {
                    document.querySelectorAll('.table-option')
        ...
        async function submitReservation() {
            ...
            const dpProofFile = document.getElementById('dpProof').files[0];
            ...
            formData.append('party_size', selectedPartySize);
